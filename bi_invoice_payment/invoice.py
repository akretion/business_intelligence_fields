...
class account_invoice(orm.Model):
    ...
    def _compute_bi_payment(self, cr, uid, ids, name, arg, context=None):
        ...
        for inv in self.browse(cr, uid, ids, context=context):
            ...
            # To test the down payment algo, you have to consider at least
            # these scenarios :
            # Scenario 1 :
            ...
            if inv.type in ('in_invoice', 'out_invoice'):
                for payment in inv.payment_ids:
                    if inv.reconciled:
                        if payment.date > max_date_all_journals:
                            max_date_all_journals = payment.date
                        # 'general' is usefull for SEPA or LCR payment with a transfer move
                        if payment.journal_id.type in ('cash', 'bank', 'general') and payment.date > max_date_bank_journal:
                            max_date_bank_journal = payment.date
                        if not payment_journal_id_to_write:
                            payment_journal_id_to_write = payment.journal_id.id

                    if payment.journal_id.type in ('cash', 'bank', 'general') and payment.date <= inv.date_invoice:
                        total_down_pay += payment.credit - payment.debit

                # We have a final payment date only when the invoice is reconciled
                ...
                if inv.reconciled and inv.payment_ids:
                    # If we have at least one pay line in bank/cash journal, we take the max
                    # date of the payment lines in bank/cash journal
                    if max_date_bank_journal:
                        date_final_payment_to_write = max_date_bank_journal
                    ...
                    final_date_dt = datetime.strptime(
                        date_final_payment_to_write, DEFAULT_SERVER_DATE_FORMAT)
                    if inv.date_due and date_final_payment_to_write > inv.date_due:
                        due_date_dt = datetime.strptime(
                            inv.date_due, DEFAULT_SERVER_DATE_FORMAT)
                        overdue_delay_days_to_write = (final_date_dt - due_date_dt).days
                    ...
                    if date_final_payment_to_write > inv.date_invoice:
                        ...
                        payment_delay_days_to_write = (final_date_dt - invoice_date_dt).days

                ctx_date = context.copy()
                ctx_date['date'] = inv.date_invoice
                amount_total_company_currency = self.pool['res.currency'].compute(
                    cr, uid, inv.currency_id.id, inv.company_id.currency_id.id,
                    inv.amount_total, context=ctx_date)
    ...
    def _bi_get_invoice_from_line(self, cr, uid, ids, context=None):
        ...
        return self.pool['account.invoice']._get_invoice_from_line(cr, uid, ids, context=context)

    def _bi_get_invoice_from_reconcile(self, cr, uid, ids, context=None):
        return self.pool['account.invoice']._get_invoice_from_reconcile(cr, uid, ids, context=context)
