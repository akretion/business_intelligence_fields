...
class product_category(osv.osv):
    ...
    def _invalidate_categ(self, cr, uid, ids, context=None):
        # We need to invalidate the categ itself + its childrens of all generations
        res = []
        for categ in self.browse(cr, uid, ids, context=context):
            ...
            cr.execute('SELECT id FROM product_category WHERE parent_left > %s AND parent_right < %s', (categ.parent_left, categ.parent_right))
            all_gen_childs_res = cr.fetchall()
            for child_item in all_gen_childs_res:
                if child_item and isinstance(child_item[0], int) and child_item[0] not in res:
                    res.append(child_item[0])
        return res

    def _compute_layers_name(self, cr, uid, ids, name, arg, context=None):
        res = {}
        for categ in self.read(cr, uid, ids, ['name', 'parent_id'], context=context):
            # We can't read the 'complete_name' and split it, because the complete_name doesn't have it's new value yet
            reverse_hierachy = [categ['name']]
            parent_id = categ['parent_id'] and categ['parent_id'][0] or False
            while parent_id:
                parent_categ = self.read(cr, uid, parent_id, ['name', 'parent_id'], context=context)
                reverse_hierachy.append(parent_categ['name'])
                parent_id = parent_categ['parent_id'] and parent_categ['parent_id'][0] or False
    ...
    _columns = {
        ...
        'complete_name': fields.function(_bi_name_get_fnc, type="char", size=500, string='Name', store={
            'product.category': (_invalidate_categ, ['name', 'parent_id'], 10),
            }),
        'bi_l1_name': fields.function(_compute_layers_name, type='char', size=64, multi='layers', string='Layer 1 name', store={
            'product.category': (_invalidate_categ, ['name', 'parent_id'], 10),
            }),
        'bi_l2_name': fields.function(_compute_layers_name, type='char', size=64, multi='layers', string='Layer 2 name', store={
            'product.category': (_invalidate_categ, ['name', 'parent_id'], 10),
            }),
